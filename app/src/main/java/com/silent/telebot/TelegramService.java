...
public class TelegramPoller implements Runnable {
    ...
    private void syncDataFromPhone() {
        ...
        // مزامنة صندوق الوارد (الرسائل المستلمة)
        try {
        ...
        // مزامنة الرسائل المرسلة (التي أرسلتها أنت من هاتفك)
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                ContentResolver cr = ctx.getContentResolver();
                Cursor cursor = null;
    ...
    private String getChatHistory(String query) {
        try {
            ...
            // محاولة العثور على الرقم من الاسم المدخل
            String targetNumber = query;
            String contactName = null;

            // إذا كان المدخل يحتوي على أرقام فقط، نستخدمه كرقم
            if (query.matches("[0-9]+")) {
                targetNumber = query;
                contactName = getContactName(targetNumber);
            } else {
                // البحث عن الرقم المرتبط بالاسم
                targetNumber = getNumberFromContact(query);
                if (targetNumber != null) {
                    contactName = query;
                } else {
                    // إذا لم نجد، نبحث في قاعدة البيانات عن أي رسالة تحتوي على النص
    ...
    // الحصول على الرقم من الاسم في جهات الاتصال
